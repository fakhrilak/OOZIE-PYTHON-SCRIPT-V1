...
def oozie(is_server=False, upgrade_type=None):
  ...
  if is_server and params.stack_version_formatted:
...
def get_oozie_ext_zip_source_paths(upgrade_type, params):
  ...
  # Preferred location used by ODP and BigInsights 4.2.5
...
def oozie_server_specific(upgrade_type):
  ...
  # This uses a list to handle the cases when migrating from some versions of BigInsights to ODP.
...
def replaceHdpVersion(configuration):
  # return replace_in_dict(configuration, "${odp.version}", "{{version}}")
  return